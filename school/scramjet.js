        ...
        protocol.get(reqOptions, (res) => {
          ...
          res.on('end', async () => {
            try {
              ...
              const isText = /^(text\/|application\/(json|xml|javascript|xhtml\+xml))/i.test(contentType) || contentType.includes('html');

            resolve({
                body: isText ? bodyBuffer.toString('utf8') : bodyBuffer.toString('utf8'),
                contentType,
                headers: res.headers,
